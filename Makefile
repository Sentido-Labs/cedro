...
release: bin/$(NAME) bin/$(NAME)cc
	@if which valgrind; then valgrind --leak-check=yes bin/$(NAME) src/$(NAME).c >/dev/null; fi
	@if which valgrind; then valgrind --leak-check=yes bin/$(NAME)cc src/$(NAME).c -I src -o /dev/null; fi
...
check: bin/$(NAME)
	...
	valgrind --leak-check=yes bin/$(NAME) src/cedrocc.c >/dev/null
